...
export   ROMS_APPLICATION=CCHANNEL
    ...
    ifort )
      if [ "${which_MPI}" = "mpich" ]; then
        ...
        #export PATH=/opt/intelsoft/openmpi/bin:$PATH
         export PATH=/opt/scyld/openmpi/1.10.2/intel/bin:$PATH
      fi
      ;;
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            ...
            export     NC_CONFIG=/usr/local/pkg/data/netCDF-Fortran/4.4.4-pic-intel-2016b/bin/nf-config
            export NETCDF_INCDIR=/usr/local/pkg/data/netCDF-Fortran/4.4.4-pic-intel-2016b/include
          fi
        else
          export       NC_CONFIG=/usr/local/pkg/data/netCDF-Fortran/4.4.4-pic-intel-2016b/bin/nf-config
          export   NETCDF_INCDIR=/usr/local/pkg/data/netCDF-Fortran/4.4.4-pic-intel-2016b/include
    ...
    pgi )
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          ...
          export       NC_CONFIG=/usr/local/pkg/netcdf/netcdf-4.3.0.pgi-13.4/bin/nc-config
          export   NETCDF_INCDIR=/usr/local/pkg/netcdf/netcdf-4.3.0.pgi-13.4/include
        fi
      else
    ...
    gfortran )
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          ...
          export       NC_CONFIG=/usr/local/pkg/netcdf/netcdf-4.3.0.gnu-4.7.3/bin/nc-config
          export   NETCDF_INCDIR=/usr/local/pkg/netcdf/netcdf-4.3.0.gnu-4.7.3/include
        fi
      else
	export         NC_CONFIG=/usr/local/pkg/netcdf/netcdf-4.3.0.gnu-4.7.3/bin/nc-config
        export     NETCDF_INCDIR=/usr/local/pkg/netcdf/netcdf-4.3.0.gnu-4.7.3/include
        export     NETCDF_LIBDIR=/usr/local/pkg/netcdf/netcdf-4.3.0.gnu-4.7.3/lib
      fi
      ;;
...
# etc).
 ...
 export MY_ANALYTICAL_DIR=${MY_PROJECT_DIR}/anafile
